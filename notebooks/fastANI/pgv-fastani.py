...
from pygenomeviz.align import AlignCoord, AlignToolBase
...
def run(
    ...
    reuse: bool = False,
    # FastANI comparison options
    frag_length: int = 3000,
    ...
    """Run genome visualization workflow using FastANI
    ...
    frag_length : float, optional
        FastANI comparison fragment length
    ...
    gv : GenomeViz
        GenomeViz instance
    """
    # Check FastANI installation
    FastAni.check_installation()
    ...
    if align_coords_file.exists() and reuse:
        print("Reuse previous FastANI result.\n")
        align_coords = AlignCoord.read(align_coords_file)
    else:
        print("Run FastANI comparison.\n")
        with TemporaryDirectory() as tmpdir:
            fastani = FastAni(gbk_list, tmpdir, frag_length, thread_num)
            align_coords = fastani.run()
...
def get_args(cli_args: list[str] | None = None) -> argparse.Namespace:
    ...
    args : argparse.Namespace
        Argument parameters
    """
    parser = get_argparser(prog_name="FastANI")
    ...
    # FastANI comparison options
    #######################################################
    fastani_alignment_opts = parser.add_argument_group("FastANI comparison Options")
    default_frag_length = 3000
    fastani_alignment_opts.add_argument(
        "--frag_length",
        type=int,
        help=f"Fragment length (Default: {default_frag_length})",
        default=default_frag_length,
        metavar="",
    )
...
class FastAni(AlignToolBase):
    ...
    def run(self) -> list[AlignCoord]:
        ...
        """
        # Make genome fasta files from Genbank files
